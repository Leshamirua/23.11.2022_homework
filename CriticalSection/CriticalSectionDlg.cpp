...
DWORD WINAPI WriteToFiles(LPVOID lp)
{
	...
	CopyFile(L"C:\\Users\\danya\\Downloads\\Критические секции\\CriticalSection\\CriticalSection\\Copy.txt", 
		L"C:\\Users\\danya\\Downloads\\Критические секции\\CriticalSection\\CriticalSection\\Copy2.txt", 0);
	if (create2.is_open()) {
		MessageBox(0, TEXT("Копия 2 создана"), TEXT(""), MB_OK);
	}
	create2.close();
	
	ofstream create3(TEXT("Copy3.txt"));
	CopyFile(L"C:\\Users\\danya\\Downloads\\Критические секции\\CriticalSection\\CriticalSection\\Copy.txt",
		L"C:\\Users\\danya\\Downloads\\Критические секции\\CriticalSection\\CriticalSection\\Copy3.txt", 0);
	if (create3.is_open()) {
		MessageBox(0, TEXT("Копия 3 создана"), TEXT(""), MB_OK);
	}
	create3.close();
